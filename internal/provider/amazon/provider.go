...
func init() {
	provider.Register("amazon", NewProvider)
}
...
func (p *Provider) Tags() []string {
	return []string{"vulnerability", "amazon", "rpm"}
}

func (p *Provider) Update(ctx context.Context, lastUpdated *time.Time) ([]string, int, error) {
	p.Logger().InfoContext(ctx, "starting Amazon Linux provider update")
	...
	if err != nil {
		return nil, 0, fmt.Errorf("fetch Amazon Linux advisories: %w", err)
	}

	p.Logger().InfoContext(ctx, "fetched Amazon Linux advisory records", "count", len(records))
	...
	p.Logger().InfoContext(ctx, "wrote Amazon Linux advisories to storage", "count", count)
