...
func (m *Manager) Get(ctx context.Context) (map[string]map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("download chainguard security db: %w", err)
	}

	records, err := m.parse(db)
	if err != nil {
		return nil, fmt.Errorf("parse chainguard security db: %w", err)
	}
...
func (m *Manager) download(ctx context.Context) (*secDB, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("fetch chainguard security data: %w", err)
	}
	...
	if err := os.WriteFile(destPath, body, 0644); err != nil {
		return nil, fmt.Errorf("save chainguard security data: %w", err)
	}
				...
				if _, exists := result[vid]; !exists {
					result[vid] = map[string]interface{}{
						"Vulnerability": map[string]interface{}{
							...
							"Link":          fmt.Sprintf("https://images.chainguard.dev/security/%s", vid),
							"Severity":      "Unknown",
							"FixedIn":       []interface{}{},
						},
					}
