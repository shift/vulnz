...
const (
	DefaultURL = "https://packages.mini.dev/advisories/secdb/security.json"
	Namespace  = "minimos:rolling"
)
...
func init() {
	provider.Register("minimos", NewProvider)
}
...
func (p *Provider) Tags() []string {
	return []string{"vulnerability", "minimos", "apk"}
}

func (p *Provider) Update(ctx context.Context, lastUpdated *time.Time) ([]string, int, error) {
	p.Logger().InfoContext(ctx, "starting minimos provider update")
	...
	if err != nil {
		return nil, 0, fmt.Errorf("fetch minimos data: %w", err)
	}

	p.Logger().InfoContext(ctx, "fetched minimos records", "count", len(records))
	...
	for vulnID, record := range records {
		envelope := &storage.Envelope{
			...
			Identifier: fmt.Sprintf("minimos:rolling:%s", vulnID),
			Item:       record,
		}
	...
	p.Logger().InfoContext(ctx, "wrote minimos records to storage", "count", count)
