...
var _ = Describe("Minimos Manager", func() {
	var (
		manager    *minimos.Manager
		tempDir    string
	...
	BeforeEach(func() {
		var err error
		tempDir, err = os.MkdirTemp("", "minimos-manager-test-*")
		...
		manager = minimos.NewManager(testServer.URL, config)
	})
	...
	Context("when fetching minimos data", func() {
		...
		It("should create proper vulnerability records with correct namespace", func() {
			...
			Expect(vuln["NamespaceName"]).To(Equal("minimos:rolling"))
		...
		It("should set VersionFormat to apk for all FixedIn entries", func() {
			...
			for vulnID, record := range records {
				...
				for _, fi := range fixedInList {
					...
					Expect(entry["NamespaceName"]).To(Equal("minimos:rolling"), "Vulnerability %s should have NamespaceName=minimos:rolling", vulnID)
				}
			}
		})
