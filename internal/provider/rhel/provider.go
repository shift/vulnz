...
func init() {
	provider.Register("rhel", NewProvider)
}
...
func (p *Provider) Tags() []string {
	return []string{"vulnerability", "os", "incremental", "large"}
}

func (p *Provider) Update(ctx context.Context, lastUpdated *time.Time) ([]string, int, error) {
	p.Logger().InfoContext(ctx, "starting RHEL provider update")
	...
	records, urls, err := p.manager.Get(ctx)
	if err != nil {
		return nil, 0, fmt.Errorf("fetch RHEL advisories: %w", err)
	}

	p.Logger().InfoContext(ctx, "fetched RHEL advisories", "count", len(records))
	...
	for _, record := range records {
		...
		vulnName := strings.ToLower(record.Name)
		...
		if err := storageBackend.Write(ctx, envelope); err != nil {
			p.Logger().WarnContext(ctx, "failed to write record", "identifier", identifier, "error", err)
			continue
		}
		count++
	}

	p.Logger().InfoContext(ctx, "wrote RHEL records to storage", "count", count)
