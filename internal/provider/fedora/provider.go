...
const (
	DefaultURL = "https://bodhi.fedoraproject.org"
...
func init() {
	provider.Register("fedora", NewProvider)
}

func NewProvider(config provider.Config) (provider.Provider, error) {
	manager := NewManagerWithURL(DefaultURL, config)
...
func (p *Provider) Tags() []string {
	return []string{"vulnerability", "fedora", "rpm"}
}

func (p *Provider) Update(ctx context.Context, lastUpdated *time.Time) ([]string, int, error) {
	p.Logger().InfoContext(ctx, "starting fedora provider update")
	...
	if err != nil {
		return nil, 0, fmt.Errorf("fetch fedora data: %w", err)
	}

	p.Logger().InfoContext(ctx, "fetched fedora records", "count", len(records))
	...
	for vulnID, record := range records {
		envelope := &storage.Envelope{
			Schema:     SchemaURL,
			Identifier: fmt.Sprintf("fedora:%s", vulnID),
			Item:       record,
		}

		if err := storageBackend.Write(ctx, envelope); err != nil {
			p.Logger().WarnContext(ctx, "failed to write record", "vuln", vulnID, "error", err)
			continue
		}
		count++
	}

	p.Logger().InfoContext(ctx, "wrote fedora records to storage", "count", count)
