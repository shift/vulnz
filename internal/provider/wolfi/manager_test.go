...
var _ = Describe("Wolfi Manager", func() {
	var (
		manager    *wolfi.Manager
		tempDir    string
	...
	BeforeEach(func() {
		var err error
		tempDir, err = os.MkdirTemp("", "wolfi-manager-test-*")
		...
		manager = wolfi.NewManager(testServer.URL, config)
	})
	...
	Context("when fetching wolfi data", func() {
		...
		It("should create proper vulnerability records with correct namespace", func() {
			...
			Expect(vuln["NamespaceName"]).To(Equal("wolfi:rolling"))
		...
		It("should set VersionFormat to apk for all FixedIn entries", func() {
			...
			for vulnID, record := range records {
				...
				for _, fi := range fixedInList {
					...
					Expect(entry["NamespaceName"]).To(Equal("wolfi:rolling"), "Vulnerability %s should have NamespaceName=wolfi:rolling", vulnID)
				}
			}
		})
