...
func (m *Manager) Get(ctx context.Context) (map[string]map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("download wolfi security db: %w", err)
	}

	records, err := m.parse(db)
	if err != nil {
		return nil, fmt.Errorf("parse wolfi security db: %w", err)
	}
...
func (m *Manager) download(ctx context.Context) (*secDB, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("fetch wolfi security data: %w", err)
	}
	...
	if err := os.WriteFile(destPath, body, 0644); err != nil {
		return nil, fmt.Errorf("save wolfi security data: %w", err)
	}
