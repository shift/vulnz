...
var _ = Describe("AlmaLinux Provider", func() {
	var (
		almaProvider provider.Provider
		tempDir      string
		logger       *slog.Logger
	...
	BeforeEach(func() {
		var err error
		tempDir, err = os.MkdirTemp("", "alma-provider-test-*")
		...
		almaProvider, err = alma.NewProvider(config)
		Expect(err).NotTo(HaveOccurred())
		Expect(almaProvider).NotTo(BeNil())
	})
	...
	Context("provider interface implementation", func() {
		It("should return correct name", func() {
			Expect(almaProvider.Name()).To(Equal("alma"))
		})

		It("should implement TagsProvider interface", func() {
			tp, ok := almaProvider.(provider.TagsProvider)
			Expect(ok).To(BeTrue())

			tags := tp.Tags()
			Expect(tags).To(ContainElement("vulnerability"))
			Expect(tags).To(ContainElement("alma"))
	...
	Context("provider registration", func() {
		...
		It("should appear in provider list", func() {
			...
			Expect(providers).To(ContainElement("alma"))
		})
	...
	Context("when updating vulnerability data", func() {
		It("should respect context cancellation", func() {
			...
			_, _, err := almaProvider.Update(ctx, nil)
			Expect(err).To(HaveOccurred())
		})

		It("should handle first run with no previous update", func() {
			Skip("Requires network access to AlmaLinux OSV database")

			ctx := context.Background()
			urls, count, err := almaProvider.Update(ctx, nil)

			Expect(err).NotTo(HaveOccurred())
			Expect(urls).To(HaveLen(1))
			Expect(urls[0]).To(ContainSubstring("github.com/AlmaLinux/osv-database"))
	...
	Context("when integrating with storage backend", func() {
		...
		It("should support SQLite storage", func() {
			...
			config.Storage.Path = tempDir + "/alma.db"
			p, err := alma.NewProvider(config)
			Expect(err).NotTo(HaveOccurred())
			Expect(p).NotTo(BeNil())
		})
