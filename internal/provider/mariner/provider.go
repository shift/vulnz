...
func init() {
	provider.Register("mariner", NewProvider)
}

func NewProvider(config provider.Config) (provider.Provider, error) {
	manager := NewManager(config, defaultVersions)
...
func (p *Provider) Tags() []string {
	return []string{"vulnerability", "mariner", "rpm", "oval"}
}

func (p *Provider) Update(ctx context.Context, lastUpdated *time.Time) ([]string, int, error) {
	p.Logger().InfoContext(ctx, "starting mariner provider update")
	...
	if err != nil {
		return nil, 0, fmt.Errorf("fetch mariner data: %w", err)
	}

	p.Logger().InfoContext(ctx, "fetched mariner records", "count", len(records))
	...
	p.Logger().InfoContext(ctx, "wrote mariner records to storage", "count", count)
