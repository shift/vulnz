...
func init() {
	provider.Register("epss", NewProvider)
}
...
func (p *Provider) Tags() []string {
	return []string{"auxiliary", "epss", "exploit-prediction"}
}

func (p *Provider) Update(ctx context.Context, lastUpdated *time.Time) ([]string, int, error) {
	p.Logger().InfoContext(ctx, "starting EPSS provider update")
	...
	if err != nil {
		return nil, 0, fmt.Errorf("fetch EPSS data: %w", err)
	}

	p.Logger().InfoContext(ctx, "fetched EPSS records", "count", len(records))
	...
	for cveID, record := range records {
		envelope := &storage.Envelope{
			...
			Identifier: fmt.Sprintf("epss:%s", cveID),
			Item:       record,
		}
	...
	p.Logger().InfoContext(ctx, "wrote EPSS records to storage", "count", count)
