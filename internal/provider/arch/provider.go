...
func init() {
	provider.Register("arch", NewProvider)
}
...
func (p *Provider) Tags() []string {
	return []string{"vulnerability", "arch", "pacman"}
}

func (p *Provider) Update(ctx context.Context, lastUpdated *time.Time) ([]string, int, error) {
	p.Logger().InfoContext(ctx, "starting arch provider update")
	...
	if err != nil {
		return nil, 0, fmt.Errorf("fetch arch data: %w", err)
	}

	p.Logger().InfoContext(ctx, "fetched arch records", "count", len(records))
	...
	for id, record := range records {
		envelope := &storage.Envelope{
			Schema:     SchemaURL,
			Identifier: fmt.Sprintf("%s:%s", Namespace, id),
			Item:       record,
		}

		if err := storageBackend.Write(ctx, envelope); err != nil {
			p.Logger().WarnContext(ctx, "failed to write record", "id", id, "error", err)
			continue
		}
		count++
	}

	p.Logger().InfoContext(ctx, "wrote arch records to storage", "count", count)
